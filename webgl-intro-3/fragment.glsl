...
void main() {
    float oscillatingTime = sin(u_time) * 0.5 + 0.5;
    vec2 normCoord = gl_FragCoord.xy/u_resolution;
    vec2 normMouse = u_mouse/u_resolution;
    gl_FragColor = vec4(oscillatingTime, normCoord.x - normMouse.x, normCoord.y - normMouse.y, 1.0);

    // uncomment me for devil magic
